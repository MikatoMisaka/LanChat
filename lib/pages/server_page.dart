...
class _ServerPageState extends State<ServerPage> {
  ...
  Future<void> _restore() async {
    ...
    if (!mounted || password == null || accessCode == null) return;
  ...
  Future<void> _addServer() async {
    ...
    try {
      ...
      await _connect(profile, result['password']!, result['accessCode']!);
    } catch (error) {
      if (mounted) setState(() => _error = '$error');
    }
  }
  ...
  Future<void> _switchServer(ServerProfile profile) async {
    final password = await _profilesStore.passwordFor(profile.id);
    final accessCode = await _profilesStore.accessCodeFor(profile.id);
    if (password == null || accessCode == null) {
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              obscureText: true,
              decoration: const InputDecoration(labelText: '服务器接入码'),
            ),
          ],
        ),
      ),
