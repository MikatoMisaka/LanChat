...
void main() {
  test('MessageHeader 编解码 roundtrip', () {
    final h = MessageHeader(type: FrameType.text, msgId: 'abc', ts: 123);
    final map = h.toMap();
  ...
  test('FrameCodec 文本帧编码', () {
    ...
    final headLen = ByteData.sublistView(bytes, 0, 4).getUint32(0, Endian.big);
  ...
  test('FrameReader 文件帧流式落盘 + 同连接文本帧', () async {
    ...
    // 同连接再跟一帧文本，验证状态机复位
    ctrl.add(
      FrameCodec.encodeText(
        MessageHeader(type: FrameType.text, msgId: 't1', ts: 2),
        'hello',
      ),
    );
    await ctrl.close();
  ...
  test(
    'FrameReader rejects an oversized buffered frame before allocating it',
    () async {
      final ctrl = StreamController<Uint8List>();
      var aborted = false;
      FrameReader(
      ...
      final header = MessageHeader(
        ...
        msgId: 'too-large',
        ts: 1,
        size: FrameReader.maxBufferedBytes + 1,
      );
      ctrl.add(FrameCodec.encodeHeaderOnly(header));
      await ctrl.close();
      await Future<void>.delayed(const Duration(milliseconds: 50));
  ...
  test('DiscoveryPayload rejects invalid identity and port fields', () {
    expect(
      DiscoveryPayload.decode(
        jsonEncode({'id': '', 'name': 'peer', 'ip': '1.2.3.4', 'tcpPort': 1}),
      ),
      isNull,
    );
    expect(
